...
public class GPSDataCache: ICloneable, IPersistent
{
...
public GPSDataCache ()
{
    m_Storage = new List<ObjectPositions> ();
}
...
public GPSDataCache (GPSDataCache other)
{
    ...
    m_Storage = new List<ObjectPositions> (size);
    for (int idx = 0; idx < size; ++idx)
    {
        ObjectPositions positions = other.m_Storage [idx];
        if (null != positions)
        {
...
/// есть сведения о таком клиенте, то они дополняются.
/// - дубликаты в хранилище не заводятся. После добавления хранилище 
/// сортируется по идетификатору клиентов.
/// </summary>
...
protected List<ObjectPositions> m_Storage;
    public List<ObjectPositions> Storage
    {
