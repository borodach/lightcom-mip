...
/*
 * $History: ObjectPositions.cs $
 * 
 * *****************  Version 5  *****************
 * User: Sergey       Date: 2.04.07    Time: 23:34
 ...
 * 
 * *****************  Version 1  *****************
 * User: Sergey       Date: 4.03.07    Time: 11:15
 * Created in $/Projects/MiP/src/Mip.Common
 * 
 * *****************  Version 8  *****************
 * User: Sergey       Date: 4.03.07    Time: 10:51
 * Updated in $/gps/Dispatcher/Cache
 * 
 * *****************  Version 7  *****************
...
public class ObjectPositions : IComparable, IPersistent, ICloneable
{
...
public ObjectPositions ()
{
    ...
    m_Storage = new List<ObjectPosition> ();
}
...
public ObjectPositions (ObjectPositions other)
{
    ...
    m_Storage = new List<ObjectPosition> (size);
...
public bool RestoreGuts (Stream stream)
{
    ...
    {
        Reset ();
        if (!RestoreGuts (stream, this.Storage))
        {
            Reset ();
            return false;
        }
...
/// <param name="stream">Поток, из которго выполняется восстановление.
/// </param>
/// <param name="storage">Контейнер для записи загруженных объектов</param>
/// <returns>true, если операция выполнилась успешно.</returns>
public bool RestoreGuts (Stream stream, ICollection<ObjectPosition> storage)
{
    try
    {
        ...
        //if (storage.Capacity < nCount) storage.Capacity = storage.Capacity + nCount;
        for (int nIdx = 0; nIdx < nCount; ++nIdx)
        {
            ObjectPosition obj = new ObjectPosition ();
...
public ObjectPositions Extract (DateTime from, DateTime to, bool bDeepCopy)
{
    ...
    int fromIdx = m_Storage.BinarySearch (0, m_Storage.Count, position, null);        
    if (-count - 1 == fromIdx) return extractedPositios;
    if (fromIdx < 0) fromIdx = -1 - fromIdx;

    position.Timestamp = to;
    int toIdx = m_Storage.BinarySearch (0, m_Storage.Count, position, null);        
    ...
    if (bDeepCopy)
    {
        ...
        {
            extractedPositios.m_Storage.Add (m_Storage [idx].Clone () as ObjectPosition);
        }                
    }
    else
    {
...
public void Add (ObjectPositions positions)
{
    ...
    LinkedList<ObjectPosition> newItems = new LinkedList<ObjectPosition> ();
    for (int idx = 0; idx < count; ++ idx)
    {
        ObjectPosition obj = positions [idx];
        ...
        {
            newItems.AddLast (obj);            
        }
    }

    m_Storage.AddRange (newItems);
...
public int GetIdxAfterTime (DateTime time)
{
    ...
    int count = m_Storage.Count;
    if (count <= 0) return -1;
int idx = m_Storage.BinarySearch (0, m_Storage.Count, position, null);
    if (idx < 0) idx = -1 - idx;
    for ( ; idx < count; ++idx)
    {
...
///
    ...
    protected List<ObjectPosition> m_Storage;
    public List<ObjectPosition> Storage
    {
