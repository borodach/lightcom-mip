 ...
 * 
 * *****************  Version 7  *****************
 * User: Sergey       Date: 18.11.06   Time: 19:03
 * Updated in $/gps/Dispatcher/Cache
 * Добавлено свойство "Курс"
 * 
 * *****************  Version 6  *****************
...
public class ObjectPosition: IComparable, IPersistent, ICloneable
{
...
public bool SaveGuts (Stream stream)
{
    try
    {
        ...
        Utils.Write (stream, (int) (m_Course * 100));
    }
    catch (Exception)
    {
        return false;
...
public bool RestoreGuts (Stream stream)
{
    try
    {
        ...
        DateTime tmpTime = Utils.ReadDateTime (stream);
        m_Timestamp = new DateTime (tmpTime.Ticks, DateTimeKind.Utc);            

        if (nVersion > 1)
        {
            m_Course = ((double) Utils.ReadInt (stream)) / 100;
        }
        else
        {
            m_Course = 0;
...
protected const int m_nLatestVersion = 2;
}
